...
def test_sentence_type_appended_to_source_id():
    ...
    for call in session.declare_publisher.call_args_list:
        key_expr = call[0][0]
        assert key_expr.endswith("/gps/test/GGA"), f"Expected key to end with /gps/test/GGA, got: {key_expr}"
...
def test_different_sentences_produce_different_keys():
    ...
    location_keys = [k for k in key_exprs if "location_fix" in k and "hdop" not in k and "satellites" not in k and "undulation" not in k]
