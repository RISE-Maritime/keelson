...
health, output artifacts), not a wire concern. A consumer that also serves
an RPC is no longer pure: `keelson2mcap` serves `configurable/v1` for its
key set, so it declares a source token and a `configuration_json` subject
token, and gets its interface token from `serve_rpc`. Don't gate on external-system
...
| **pc** | Compute host health via `psutil`. Single binary `pc2keelson`. Publishes only the nine vessel-level decision subjects (host identity, CPU load/temperature, memory/swap %, disk used %/free bytes, NIC up) — finer host metrics belong to node-exporter, not the bus. Collectors (`pc/collectors.py`) import no zenoh; publishing (`pc/publishing.py`) imports no psutil. Missing beats wrong: unexposed metrics are not published. In Docker needs `--pid host`, host `/proc` + root bind mounts with `--procfs-path`/`--host-root`. |
| **container_control** | Host container inspection/control over the Docker socket. `docker2keelson` serves `container_control/v1` (list, logs, start, stop, restart, remove) and publishes `container_status` (on by default), `container_stats` (opt-in, `--publish-stats`) and per-container `log_message` (opt-in, `--follow-logs`). Read-only unless `--allow-control` + `--allow GLOB`; `remove` is gated separately by `--allow-remove GLOB`; refuses to act on its own container even when a glob matches. Also `container-control-cli` (ops client) and `container-control-healthcheck` (compose probe, never imports zenoh). Only `container_control/backend.py` imports `docker` — the package is not named `docker/` because that would shadow the PyPI package. Needs `/var/run/docker.sock` and `init: true`. |
