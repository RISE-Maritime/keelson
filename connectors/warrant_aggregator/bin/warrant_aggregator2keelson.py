    ...
    with zenoh.open(zconf) as session:
        ...
        def sink(event: dict) -> None:
            if event["kind"] == "snapshot":
                event = {**event, "policy_config_digest": digest.hex()}
            if event["kind"] == "level":
        ...
        def on_sample(sample: zenoh.Sample) -> None:
            try:
                _received, _enclosed, payload = keelson.uncover(
                ...
                return
            with lock:
                engine.feed(time.time_ns(), msg)
                if state["level_dirty"]:
                    state["level_dirty"] = False
                    _publish_authority()

        def _publish_authority() -> None:
            msg = operational_authority_from_state(
                engine, time.time_ns(), args.policy_id, digest
            )
