...
def test_connector_advertises_every_subject_it_publishes(
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            if "entity_health" in advertised:
                break
            time.sleep(0.5)

        assert "entity_health" in advertised, f"advertised: {sorted(advertised)}"
        # The determination moved to composite_aggregator, which declares its
        # own token: this connector must no longer advertise it.
        assert "operational_authority" not in advertised, (
            "entity_health still advertises operational_authority — the "
            f"determination belongs to composite_aggregator now: {sorted(advertised)}"
        )
    finally:
        if health is not None:
            health.stop()
