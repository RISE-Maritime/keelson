...
def run(args: argparse.Namespace):
    with (
        ...
        for received_at, key, envelope in klog_read_message(fhi):
            with ignore(Exception):
                ...
                try:
                    ...
                    logger.error(
                        "Topic %s did not contain a valid keelson.Envelope",
                        key,
                    )
                    continue
                ...
                try:
                    ...
                    warnings.warn(
                        f"Received key did not match the expected format (skipping): {key}"
                    )
                    invalid_key_count += 1
                    continue
        ...
        # Log summary
        logger.info(
            "Conversion summary: processed=%d, invalid_keys=%d",
            processed_count,
            invalid_key_count,
        )
